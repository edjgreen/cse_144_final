# cse_144_final
Click below to open the notebook in Google Colab:

[Open in Colab](https://colab.research.google.com/github/edjgreen/cse_144_final/blob/main/CSE-144_HW2-1_colab.ipynb)  

Run each cell in CSE-144_HW2-1.ipynb in sequential order (or just click run all)  
I recommend switching runtime type to GPU for cuda capabilities  
original dataset in test and train folder  
384x384 resized dataset in preprocessed and preprocessed_test folders  

<img width="689" height="117" alt="Screenshot 2026-06-08 144637" src="https://github.com/user-attachments/assets/d2d6bb22-1e9c-423c-9d71-c212ca3440d6" />

Link to Kaggle submitted weights:  
https://drive.google.com/drive/folders/1xdbHavXD-2T7ftaiNFynATA3lnrDOIr7?usp=sharing  
